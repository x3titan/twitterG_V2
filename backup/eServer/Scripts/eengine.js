...
function addText(div, text, hAlign, vAlign, font, size, color, weight) {
    ...
    var r = eb.getDivRectSetting(div);
...
function addImageOnLoad(na, img) {
    ...
    if (!img.parentNode.style) return;

    var div = img.parentNode;
    if (div.c_imgOld) {
        div.removeChild(div.c_imgOld);
        div.c_imgOld = undefined;
    }

    var imgHeight;
    var ratio;
    var firstLoad = false;
    var imgMode = img.parentNode.imgMode ? img.parentNode.imgMode : "trim";
    if (eb.isEmpty(img.sw)) {
        img.sw = img.width;
        img.sh = img.height;
        firstLoad = true;
    }
    var rect = eb.getDivRectSetting(img.parentNode);
    if (imgMode == "trim") {
        if (rect.w / img.sw > rect.h / img.sh) {
            ratio = rect.w / img.sw;
            img.width = rect.w;
            img.height = parseInt(img.sh * ratio + 1);
            img.style.left = "0px";
            img.style.top = parseInt(rect.h / 2 - img.height / 2) + "px";
        } else {
            ratio = rect.h / img.sh;
            img.width = parseInt(img.sw * ratio + 1);
            img.height = rect.h;
            img.style.left = parseInt(rect.w / 2 - img.width / 2) + "px";
            img.style.top = "0px";
        }
        ...
        img.style.left = parseInt(rect.w / 2 - img.sw / 2) + "px";
        img.style.top = parseInt(rect.h / 2 - img.sh / 2) + "px";
...
var ee = {
    ...
    setRect: function (panel, x, y, w, h, refresh) {
        var i, j, lastp, currentp, pos, rect = eb.getDivRectSetting(panel);
        ...
        if (eb.isEmpty(w)) w = rect.w;
        if (eb.isEmpty(h)) h = rect.h;
        eb.setDivRect(panel, x, y, w, h);
    ...
    getMaxY: function (panel, child) {
        var result = 0;
        var rect;
        for (var i = 0; i < panel.eeChildren.length; i++) {
            if ((panel.eeChildren[i] != child) && (panel.eeChildren[i].style.visibility == "hidden")) continue;
            rect = eb.getDivRectSetting(panel.eeChildren[i]);
            if (rect.y + rect.h > result) result = rect.y + rect.h;
        }
        return result;
    },
    tiledV: function (panel, sp, spTop, spBottom) {
        if (eb.isEmpty(spTop)) spTop = 0;
        if (eb.isEmpty(spBottom)) spBottom = 0;
        panel.v_tiledVSp = sp;
        panel.v_tiledVSpTop = spTop;
        panel.v_tiledVSpBottom = spBottom;
        var rect, cy = spTop;
        for (var i = 0; i < panel.eeChildren.length; i++) {
            if (panel.eeChildren[i].style.visibility == "hidden") continue;
            if (i != 0) cy += sp;
            ee.setRect(panel.eeChildren[i], null, cy, null, null);
            rect = eb.getDivRectSetting(panel.eeChildren[i]);
            cy += rect.h;
        }
        cy += spBottom;
        ee.setRect(panel, null, null, null, cy);
    },
    tiledResizeV: function (panel, level) {
        var child = null;
        for (var i = 0; i < level; i++) {
            if (!eb.isEmpty(panel.v_tiledVSp)) {
                ee.tiledV(panel, panel.v_tiledVSp, panel.v_tiledVSpTop, panel.v_tiledVSpBottom);
            } else {
                ee.resizeV(panel, 1);
            }
    ...
    resizeV: function (panel, level) {
        var child = null;
        for (var i = 0; i < level; i++) {
            var maxY = ee.getMaxY(panel, child);
            ee.setRect(panel, null, null, null, maxY);
            child = panel;
            panel = panel.parentNode;
        }
    },
    ...
    setImage: function (div, imgUrl, imgMode, alt, title) {
        if (!eb.isEmpty(div.img)) {
            ...
            div.c_imgOld = div.img;
            div.img = undefined;
        }
        if (eb.isEmpty(imgUrl)) imgUrl = "";
        ...
        eb.setImgRect(div.img, rect.w, rect.h, null, null);
    ...
    setOnClick: function (div, func) {
        ...
        if ("ontouchstart" in window) {
            ...
            eb.addEvent(div, "touchend", function (event) {
                ...
                if ((d < 20) && ((new Date()).getTime() - div.eeTouchStartTime.getTime() < 300)) {
                    if (div.eeOnClick) {
                        //setTimeout(function () { //?
    ...
    changeParent: function (control, newParent, pos) {
        var i;
    ...
    isInternalNode: function (panel, node) {
        if (eb.isEmpty(panel)) return false;
        if (panel == node) return true;
        if (panel.label) {
            if (ee.isBranchNode(panel.label.parentNode, node)) return true;
        }
        if (panel._input) {
            if (ee.isBranchNode(panel._input, node)) return true;
        }
        if (panel.img) {
            if (ee.isBranchNode(panel.img, node)) return true;
        }
        if (panel.skin) {
            for (var i = 0; i < panel.skin.length; i++) {
                if (ee.isBranchNode(panel.skin[i], node)) return true;
            }
        }
        return false;
    },
    isBranchNode: function (root, node) {
        if (eb.isEmpty(root)) return false;
        while (!eb.isEmpty(node)) {
...
ee.setAnimationEnd = function (div, func) {
    var getAniEvent = function () {
    ...
    var aniEvent = getAniEvent();
    if (aniEvent == "") return;
    if (div.f_onTransitionEnd) div.removeEventListener(aniEvent, div.f_onTransitionEnd);
    div.addEventListener(aniEvent, func, false);
    div.f_onTransitionEnd = func;
}
...
var eb = {
    ...
    getPxValue: function (value) {
        ...
        if (value.substr(value.length - 2, 2) != "px") return null;
        var r = parseInt(value.substr(0, value.length - 2));
        return r == NaN ? null : r;
    },
    getDivRectSetting: function (div) {
        var r = new Rect();
        r.x = this.getPxValue(div.style.left); r.x = r.x == null ? div.offsetLeft : r.x;
        r.y = this.getPxValue(div.style.top); r.y = r.y == null ? div.offsetTop : r.y;
        r.w = this.getPxValue(div.style.width); r.w = r.w == null ? div.offsetWidth : r.w;
        r.h = this.getPxValue(div.style.height); r.h = r.h == null ? div.offsetHeight : r.h;
        return r;
    },
    ...
    processDock: function () {
        ...
        while (modified) {
            ...
            for (i = 0; i < eb.dockItem.length; i++) {
                ...
                if (r.w < 0) r.w = 0;
                if (r.h < 0) r.h = 0;
                if (!r.equals(eb.getDivRectSetting(item))) {
                    modified = true;
